...
//        }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        PointF center = getCenterOffsets();
        final int xIncrements = 1 + getSkipWebLineCount();
        for (int i = 0; i < getChildCount(); i += xIncrements) {
            PointF p = Utils.getPosition(
                    center,
                    getYRange() * factor,
                    sliceangle * i + rotationangle);
            addRadarView(i, (int) p.x, (int) p.y, getChildAt(i));
        }
    }

    private void addRadarView(int i, int x, int y, View view) {
        int width = view.getMeasuredWidth();
        int height = view.getMeasuredHeight();
        switch (i) {
            case 0:
                view.layout(x - width / 2, y - 10 - height, x + width / 2, y - 10);
                break;
            case 1:
                view.layout(x + 10, y - height / 2, x + 10 + width, y + height / 2);
                break;
            case 2:
                view.layout(x, y + 10, x + width, y + 10 + height);
                break;
            case 3:
                view.layout(x - width, y + 10, x, y + 10 + height);
                break;
            case 4:
