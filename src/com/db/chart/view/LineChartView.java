...
public class LineChartView extends ChartView {
	...
	public LineChartView(Context context, AttributeSet attrs) {
		...
		mStyle = new Style(context.getTheme().obtainStyledAttributes(attrs,
				R.styleable.ChartAttrs, 0, 0));
		sRegionRadius = (float) getResources().getDimension(
				R.dimen.dot_region_radius);
	}
	...
	public void onDrawChart(Canvas canvas, ArrayList<ChartSet> data) {
		...
		for (int i = 0; i < data.size(); i++) {
			...
			if (lineSet.isDashed())
				mStyle.mLinePaint.setPathEffect(new DashPathEffect(new float[] {
						10, 10 }, 0));
			else
	...
	private void drawPoints(Canvas canvas, LineSet set) {
		...
		mStyle.mDotsPaint
				.setAlpha((set.getAlpha() < 1) ? (int) (set.getAlpha() * 255)
						: 255);
		...
		for (ChartEntry e : set.getEntries())
			path.addCircle(e.getX(), e.getY(), set.getDotsRadius(),
					Path.Direction.CW);
	...
	/**
	 * Credits: http://www.jayway.com/author/andersericsson/ Method responsible
	 * to draw a smooth line with the parsed screen points.
	 */
	private void drawSmoothLine(Canvas canvas, LineSet set) {
		...
		for (int i = 0; i < set.size() - 1; i++) {
			...
			startdiffX = (nextPointX - set.getEntry(si(set.size(), i - 1))
					.getX());
			startdiffY = (nextPointY - set.getEntry(si(set.size(), i - 1))
					.getY());
			...
			// Define background
			bgPath.cubicTo(firstControlX, firstControlY, secondControlX,
					secondControlY, nextPointX, nextPointY);
	 ...
	 */
	private void drawBackground(Canvas canvas, Path path, LineSet set,
			float minDisplayY) {
		...
		if (set.hasGradientFill())
			mStyle.mFillPaint.setShader(new LinearGradient(super
					.getInnerChartLeft(), minDisplayY, super
					.getInnerChartLeft(), super.getInnerChartBottom(), set
					.getGradientColors(), set.getGradientPositions(),
					Shader.TileMode.MIRROR));

		path.lineTo(set.getEntry(set.size() - 1).getX(),
	...
	private void handleAlpha(Paint paint, float alpha) {
		...
		paint.setShadowLayer(
				...
				mStyle.mShadowDy,
				Color.argb(
						((int) (alpha * 255) < mStyle.mAlpha) ? (int) (alpha * 255)
								: mStyle.mAlpha, mStyle.mRed, mStyle.mGreen,
						mStyle.mBlue));
	}

	/**
	 * Credits: http://www.jayway.com/author/andersericsson/ Given an index in
	 * points, it will make sure the the returned index is within the array.
	 */
	...
	/**
	 * Class responsible to style the LineChart! Can be instantiated with or
	 * without attributes.
	 */
	class Style {
