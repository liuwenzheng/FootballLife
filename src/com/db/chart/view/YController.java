...
/**
 * Class responsible to control vertical measures, positions, yadda yadda. If
...
public class YController {
	...
	public YController(ChartView chartView) {
		...
		// Set defaults
		step = DEFAULT_STEP;
		topSpacing = mChartView.getResources().getDimension(
				R.dimen.axis_top_spacing);
		mAxisHorPosition = 0;
		maxLabelValue = 0;
	...
	public YController(ChartView chartView, TypedArray attrs) {
		...
		topSpacing = attrs.getDimension(
				R.styleable.ChartAttrs_chart_axisTopSpacing, topSpacing);
	}

	protected void init() {

		mDistLabelToAxis = (int) mChartView.getResources().getDimension(
				R.dimen.axis_dist_from_label);
		...
		if (maxLabelValue < calcMaxY()) {
			try {
				throw new ChartException(
						"MaxAxisValue defined < than current max set value");
	...
	/**
	 * Get labels position having into account the vertical padding of text
	 * size.
	 * 
	 * @param nLabels
	 */
	private ArrayList<Float> calcLabelsPos(int nLabels) {
		...
		final int frameHeight = (int) mChartView.horController
				.getAxisVerticalPosition() - mChartView.chartTop;
		mScreenStep = (float) (frameHeight - topSpacing) / mLabels.size();

		float currPos = (float) (mChartView.horController
				.getAxisVerticalPosition() - mScreenStep);
	...
	protected float calcAxisHorizontalPosition() {
		...
		if (labelsPositioning == LabelPosition.OUTSIDE) { // TODO estes casos
															// estao estranhos
			...
			for (int i = 0; i < mLabels.size(); i++) {
				...
				aux = mChartView.style.labelPaint.measureText(Integer
						.toString(mLabels.get(i)));
	...
	protected float parseYPos(double value) {
		return (float) (mChartView.horController.getAxisVerticalPosition() - ((value * mScreenStep) / mLabels
				.get(0)));
	}
	...
	protected void draw(Canvas canvas) {
		...
		if (hasAxis)
			// Draw axis line
			canvas.drawLine(mAxisHorPosition, mChartView.chartTop,
					mAxisHorPosition,
					mChartView.horController.getAxisVerticalPosition()
		...
		if (labelsPositioning != LabelPosition.NONE) {
			...
			mChartView.style.labelPaint
					.setTextAlign((labelsPositioning == LabelPosition.OUTSIDE) ? Align.RIGHT
							: Align.LEFT);

			// Draw labels
			for (int i = 0; i < mLabels.size(); i++) {
				canvas.drawText(
						Integer.toString(mLabels.get(i)),
						mAxisHorPosition - mChartView.style.axisThickness / 2
								- mDistLabelToAxis,
						(float) labelsPos.get(i)
								+ mChartView.style.getTextHeightBounds("0") / 2,
						mChartView.style.labelPaint);
			}
		}
	}
	...
	/**
	 * Differentiates the inner left side of the chart depending if axis Y is
	 * drawn or not. If drawing axis give it gives half of the line thickness as
	 * margin. Inner Chart refers only to the area where chart data will be
	 * draw, excluding labels, axis, etc.
	 * 
	...
	public float getInnerChartBottom() {
		return mChartView.horController.getAxisVerticalPosition()
				- mChartView.style.axisThickness / 2;
	}
