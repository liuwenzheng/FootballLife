...
public class XController {
	...
	public XController(ChartView chartView) {
		...
		borderSpacing = mChartView.getResources().getDimension(
	...
	public XController(ChartView chartView, TypedArray attrs) {
		...
		borderSpacing = attrs.getDimension(
				R.styleable.ChartAttrs_chart_axisBorderSpacing, borderSpacing);
	}
	...
	protected void init() {
		...
		if (mandatoryBorderSpacing == 1)
			mandatoryBorderSpacing = (getInnerChartRight()
					- mChartView.getInnerChartLeft() - borderSpacing * 2)
	...
	/**
	 * Get labels position having into account the horizontal padding of text
	 * size.
	 * 
	 * @param nLabels
	...
	private ArrayList<Float> calcLabelsPos(int nLabels) {
		...
		if (nLabels == 1)
			result.add(mChartView.getInnerChartLeft()
					+ (getInnerChartRight() - mChartView.getInnerChartLeft())
					/ 2);
		else {
			final float screenStep = (getInnerChartRight()
					...
					// axis
					- borderSpacing * 2 - mandatoryBorderSpacing * 2)
					/ (nLabels - 1);

			float pos = mChartView.getInnerChartLeft() + borderSpacing
					+ mandatoryBorderSpacing;
			while (pos <= mChartView.chartRight - borderSpacing
					- mandatoryBorderSpacing) {
				result.add(pos);
				pos += screenStep;
			}
	...
	protected void draw(Canvas canvas) {
		...
		if (hasAxis)
			canvas.drawLine(mChartView.getInnerChartLeft(),
					...
					getAxisVerticalPosition(), mChartView.style.chartPaint);
		...
		if (labelsPositioning != LabelPosition.NONE) {
			...
			for (int i = 0; i < mChartView.data.get(0).size(); i++) {
				canvas.drawText(mChartView.data.get(0).getLabel(i),
						labelsPos.get(i), mLabelVerCoord,
	...
	protected float getAxisVerticalPosition() {
		...
		return mChartView.chartBottom
				- mChartView.style.getTextHeightBounds(mChartView.data.get(0)
						.getLabel(0)) - mDistLabelToAxis;
	}
