package com.blestep.footballlife.view;
...
/**
 * An circle view, similar to Android's ProgressBar. Can be used in 'value mode'
 * or 'spinning mode'.
 * 
 * In spinning mode it can be used like a intermediate progress bar.
 * 
 * In value mode it can be used as a progress bar or to visualize any other
 * value. Setting a value is fully animated. There are also nice transitions
 * from animating to value mode.
 * 
 * Typical use case would be to load a new value. During the loading time set
 * the CircleView to spinning. As soon as you got your nur value, just set it
 * {@link #setValueAnimated(float, long) setValueAnimated}, it automatically
 * animated.
 * 
 * 
...
public class CircleProgressView extends View {
	...
	private int mOuterColor = mBarColorStandard;
	...
	private RectF mOuterBounds = new RectF();
	// Animation
	...
	/**
	 * Indicates if the given text or the current percentage value should be
	 * shown. true: current value (percentage or value) false: given text
	 */
	...
	private void parseAttributes(TypedArray a) {
		...
		setUnitColor(a.getColor(R.styleable.CircleProgressView_unitColor, -1));
	...
	private RectF getInnerCircleRect(RectF _circleBounds) {
		...
		float scaleY = 1;
		if (isShowUnit()) {
			scaleX = 0.77f; // scaleX square to rectangle, so the longer text
							// with unit fits better
			scaleY = 1.33f;
		}
	...
	/**
	 * Set the text in the mCurrentValue bar. If no text (null or empty) is
	 * specified, * the current percentage value is used * (max value and
	 * current value are mandatory to do so). *
	 */
	...
	/**
	 * Returns the bounding rectangle of the given _text, with the size and
	 * style defined in the _textPaint centered in the middle of the _textBounds
	 * 
	 * @param _text
	 *            The text.
	 * @param _textPaint
	 *            The paint defining the text size and style.
	 * @param _textBounds
	...
	/**
	 * If true and no text was specified. The percentage (max value, current
	 * value) is shown. If false the current value is shown.
	 * 
	 * @param _showPercentAsAutoValue
	 *            bool
	 */
	 ...
	 * 
	 * @param textSize
	 *            The text size or 0 enable automatic size (default)
	 */
	 ...
	 * 
	 * @param _unitScale
	 *            The scale value between. 1 >= value > 0
	 */
	...
	/**
	 * Scale factor for main text in the center of the circle view. Only
	 * relevant if auto text size is enabled.
	 * 
	 * @param _textScale
	 *            The scale value.
	 */
	 ...
	 * 
	 * @param textColor
	 *            the color or -1 to use auto color (depending on bar colors)
	 */
	...
	/**
	 * Set the properties of the paints we're using to draw all the things.
	 * 
	...
	private void drawCircleWithNumber(Canvas canvas, float _degrees) {
		...
		if (mAutoTextSize) { // only re-calc position and size if string length
								// changed
			...
			mTextBounds = getTextBounds(text, mTextPaint, mCircleBounds); // center
																			// text
																			// in
																			// circle
		}
	...
	/**
	 * Set the value of the circle view without an animation. Stops any
	...
	/**
	 * Sets the value of the circle view with an animation. The current value is
	 * used as the start value of the animation
	 * 
	 * @param _valueTo
	 *            value after animation
	 * @param _animationDuration
	...
	private enum AnimationState {
		...
		IDLE, SPINNING, END_SPINNING, END_SPINNING_START_ANIMATING, ANIMATING

	}

	private enum AnimationMsg {

		START_SPINNING, STOP_SPINNING, SET_VALUE, SET_VALUE_ANIMATED, TICK
	...
	private static class AnimationHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			if (msgType == AnimationMsg.TICK)
				removeMessages(AnimationMsg.TICK.ordinal()); // necessary to
																// remove
																// concurrent
																// ticks.

			// if (msgType != AnimationMsg.TICK)
			// Log.d("JaGr", TAG + "LOG00099: State:" +
			// circleView.mAnimationState + "     Received: " + msgType);
			...
			case IDLE:
				...
				case SET_VALUE_ANIMATED:
					...
					break;
				case TICK:
					removeMessages(AnimationMsg.TICK.ordinal()); // remove old
																	// ticks
				...
				case TICK:
					...
					if (Math.abs(length_delta) < 1) {
						...
						circleView.mSpinningBarLengthCurrent = mSpinningBarLengthStart
								+ ((circleView.mSpinningBarLengthOrig - mSpinningBarLengthStart) * interpolatedRatio);
					} else {
			...
			case END_SPINNING_START_ANIMATING:
				...
				case TICK:
					...
					if (circleView.mSpinningBarLengthCurrent > circleView.mSpinningBarLengthOrig
					...
					// value is already animating, calc animation value and
					// reduce spinner
		 ...
		 * @param _circleView
		 *            the circle view
		 * @return false if animation still running, true if animation is
		 *         finished.
		 */
