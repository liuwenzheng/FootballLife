...
public class HistoryTabEndurance extends Fragment implements OnEntryClickListener {
	private String mLabels[];
	private String mValues[];
	private int BAR_ENDURANCE_MAX = 1;
	// private int BAR_STEP_AIM = 0;
	private ArrayList<Step> mSteps;
	private ArrayList<Step> mStepsSort;
	private SimpleDateFormat mSdf;
	private Calendar mCalendar;
	private TextView tv_history_endurance_daily, tv_history_endurance_sum;
	...
	private BarChartView bcv_endurance;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mView = inflater.inflate(R.layout.history_tab_endurance, container, false);
		initView();
		initData();
		return mView;
	}
	...
	private void initData() {
		...
		if (Float.valueOf(mStepsSort.get(0).endurance) >= BAR_ENDURANCE_MAX) {
			BAR_ENDURANCE_MAX = Float.valueOf(mStepsSort.get(0).endurance).intValue() + 1;
		}
		// else {
		...
		tv_history_endurance_sum.setText(new BigDecimal(stepSum).setScale(2,
				BigDecimal.ROUND_HALF_UP).floatValue()
				+ "");
		tv_history_endurance_daily.setText(new BigDecimal(stepSum
	...
	private void initView() {
		bcv_endurance = (BarChartView) mView.findViewById(R.id.bcv_endurance);
		tv_history_endurance_daily = (TextView) mView
				.findViewById(R.id.tv_history_endurance_daily);
		tv_history_endurance_sum = (TextView) mView
				.findViewById(R.id.tv_history_endurance_sum);
		bcv_endurance.setOnEntryClickListener(this);
	}
	...
	public void updateBarChart(int nPoints) {
			...
			if (index < nPoints && j < start) {
				...
				if (index < nPoints) {
					bar = new Bar(mLabels[j], Float.valueOf(mSteps.get(j
							- start).endurance));
					mValues[j] = mSteps.get(j - start).endurance;
				} else {
					bar = new Bar(mLabels[j], Float.valueOf(mSteps.get(j
							+ start).endurance));
					mValues[j] = mSteps.get(j + start).endurance;
		...
		bcv_endurance.addData(data);

		bcv_endurance.setBarSpacing((int) Tools.fromDpToPx(50));
		bcv_endurance.setSetSpacing(0);
		bcv_endurance.setBarBackground(false);
		bcv_endurance.setRoundCorners(0);
		// 运动目标值
		// bcv_endurance.setmThresholdText(BAR_STEP_AIM + "");
		bcv_endurance.setBorderSpacing(0).setGrid(null).setHorizontalGrid(null)
				...
				.setMaxAxisValue(BAR_ENDURANCE_MAX, 1)
	...
	private void showBarTooltip(int index, Rect rect) {
		...
		bcv_endurance.showTooltip(mBarTooltip);
		bcv_endurance.invalidate();
	}

	private void dismissBarTooltip(final int index, final Rect rect) {

		bcv_endurance.dismissTooltip(mBarTooltip);
	...
	private class StepCompare implements Comparator<Step> {
		...
		public int compare(Step lhs, Step rhs) {
			if (Float.valueOf(lhs.endurance) > Float.valueOf(rhs.endurance)) {
				return -1;
			} else if (Float.valueOf(lhs.endurance) < Float
					.valueOf(rhs.endurance)) {
				return 1;
			}
			return 0;
		}
